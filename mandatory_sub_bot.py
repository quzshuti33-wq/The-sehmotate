...
async def start(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    if not missing:
        await update.message.reply_text("✅ انت مشترك بكل المطلوب.")
    else:
        text="⚠️ لازم تشترك في التالي:\n"+"\n".join(missing)
        kb=[[InlineKeyboardButton("قناة البوت",url=f"https://t.me/{BOT_CHANNEL.lstrip('@')}"),
...
async def panel(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("لوحة الإدارة:",reply_markup=InlineKeyboardMarkup(kb))
...
async def callback(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    data=q.data
    if data in ["add_channel","add_group","add_bot",
                "del_channel","del_group","del_bot"]:
    ...
    if data=="show_all":
        ...
        await q.message.reply_document(InputFile("subscriptions.txt"),caption="📋 هذه هي القوائم الحالية.")
